...
// 손글씨체(나눔손글씨 펜) 우선, 실패 시 NotoSansKR Bold
try {
...
function drawCloud(ctx, box, color) {
  ...
  for (let i = 0; i < steps; i++) {
    ...
    const bulge = 1.03 + 0.12 * (Math.sin(bumps * a) * 0.5 + 0.5)
...
export function renderAnnotationPNG(scene) {
  ...
  // 위치: x/y(0~1 비율) 지점을 박스 중심으로, 없으면 앵커(position)를 박스 중심으로
  let cx, cy
  ...
  const inflMap = {
    cloud: [boxW * 0.05 + 10, boxH * 0.12 + 10],
    oval: [boxW * 0.17 + 10, boxH * 0.22 + 10],
    arrow_box: [16, 10],
  }
  ...
  let top = cy - boxH / 2
  const marginX = 28 + ix
  // 하단 자막(별도 렌더) 영역을 침범하지 않도록 버블 하단을 H*0.75 위로 제한
  const arrowPad = scene.arrow ? H * 0.1 : 0
  const capSafeTop = H * 0.75 - boxH - iy - arrowPad
  left = Math.max(marginX, Math.min(left, W - boxW - marginX))
  top = Math.max(40 + iy, Math.min(top, Math.min(H - boxH - 40 - iy, capSafeTop)))
  ...
  //  - 말풍선 있음: 판을 그리면 "버블 안 박스"로 이중 → 판 없이 글자 외곽선만
  const backing = scene.backing !== false
  const showPlate = bubble === 'none' && backing
  if (showPlate) {
    drawSoftBacking(ctx, [textBox[0] - 8, textBox[1] - 4, textBox[2] + 8, textBox[3] + 4],
      Math.min(boxW, boxH) * 0.4)
  }
  const strokeW = showPlate ? 0 : Math.max(4, fs * 0.09)
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    const angle = rnd(-2.2, 2.2) * (Math.PI / 180)
    ctx.save()
    ctx.translate(tx, ty)
    ctx.rotate(angle)
    if (strokeW) ctx.strokeText(lines[i], 0, 0)
    ctx.fillText(lines[i], 0, 0)
    ctx.restore()
  }

  // 데코 — 말풍선 상단 좌/우 근처에 작게 (최대 2개)
  const decoSize = Math.round(fs * 0.5)
  const bh = bubbleBox[3] - bubbleBox[1]
  const decoPos = [
    [bubbleBox[0] + decoSize * 0.3, bubbleBox[1] + bh * 0.12],
    [bubbleBox[2] - decoSize * 0.3, bubbleBox[1] + bh * 0.06],
  ]
  ;(scene.deco || []).slice(0, 2).forEach((d, i) => {
